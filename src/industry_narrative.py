...
def _build_prompt(summary: dict[str, Any]) -> str:
    ...
    return (
        ...
        "FRAMING: the page this copy lands on is titled 'AEO for "
        f"{industry}'. Treat the subject as answer engine optimisation (AEO) "
        "performance for this category. Use the term 'AEO' or 'answer engine "
        "optimisation' naturally where it fits, but do not force it into every "
        "sentence and never pad with generic AEO definitions.\n\n"
        "1. 'narrative_paragraphs': exactly 3 short paragraphs (60 to 90 words "
        "each) of OBSERVATIONS about this specific category's AEO landscape. "
        "Reference actual brand names, scores, and patterns from the "
        ...
        "outlier). No fluff.\n\n"
        "3. 'faqs': 4 to 6 Question/Answer pairs about AEO for THIS specific "
        "category. Questions should be queries a buyer or analyst might "
        f"actually ask, for example: 'Who leads AEO in {industry}?', "
        f"'What sources does AI cite most for {industry} research?', "
        f"'How do I improve AEO for a {industry} brand?'. Answers should be "
        "1 to 2 sentences grounded in the data.\n\n"
...
def _build_buyer_intro_prompt(
    ...
    return (
        f"You are writing the introduction to a public-facing page titled "
        f"'AEO for {industry_name}: AI search visibility ranking'. AEO means "
        f"answer engine optimisation: how well a brand performs when AI "
        f"engines answer buyer questions. The audience is {audience}. The "
        f"page below the intro ranks {noun} by how often AI engines (ChatGPT, "
        f"Claude, Perplexity, Gemini, Google AI) name them in answers and "
        f"cite them as sources.\n\n"
        "STYLE RULES (strict, non-negotiable):\n"
        "* Write for the buyer, not the analyst. The reader cares about "
        "  picking a brand, not about monitoraeo's methodology.\n"
        "* You may use the term 'AEO' or 'answer engine optimisation' where "
        "  it reads naturally, but never define it and never force it. The "
        "  page title already establishes the topic.\n"
        "* NEVER use em dashes (U+2014) or en dashes (U+2013). Use commas, "
        "  parentheses, or rewrite the sentence. This is a hard requirement.\n"
