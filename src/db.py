...
def init_db() -> None:
    ...
    migrations = [
        ...
        # Narrative cache + history table (SEO uniqueness work — adds
        # AI-generated per-industry commentary, FAQs, and a movers/shakers
        # diff vs the previous refresh).
        "ALTER TABLE monitor_industry_report ADD COLUMN IF NOT EXISTS narrative JSONB DEFAULT '{}'::jsonb",
        # Empty-page guardrail: noindex flag, default FALSE so existing
        # working pages stay indexed. refresh_industry sets it after every
        # successful pass based on whether any brand scored.
        "ALTER TABLE monitor_industry_report ADD COLUMN IF NOT EXISTS noindex BOOLEAN DEFAULT FALSE",
        # Buyer-intent intro: 2-3 short paragraphs rendered ABOVE the data
        # tables, framed for someone evaluating brands in this category
        # (not for the analyst evaluating monitoraeo). Generated once per
        # refresh by industry_narrative.generate_buyer_intro(). Shape:
        #   {generated_at, model, paragraphs: [str]}.
        # Empty dict for pre-buyer-intro rows; template falls back to no
        # intro and renders straight into "At a glance" as before.
        "ALTER TABLE monitor_industry_report ADD COLUMN IF NOT EXISTS buyer_intro JSONB DEFAULT '{}'::jsonb",
        # Backfill: any audited industry where NO brand has visibility>0
        # or citation>0 is empty — hide it from Google + sitemap. Idempotent:
        # next refresh will flip back to FALSE if anyone scores. Captures
        # the current 4 known empties (lawyer-dallas / lawyer-los-angeles /
        # lawyer-miami / legal-services) plus any future empties that slip
...
class IndustryReport(SQLModel, table=True):
    ...
    #   {generated_at, model, narrative_paragraphs: [str], brand_insights:
    #    [{rank, brand_name, text}], faqs: [{q, a}]}.
    # Regenerated by industry_narrative.generate() on each refresh. Empty
    # dict for pre-narrative rows; template falls back gracefully.
    narrative: dict[str, Any] = Field(default_factory=dict, sa_column=Column(JSONB))
    # Buyer-intent intro rendered ABOVE the data tables. Different from
    # `narrative` (which sits below the rankings as analyst commentary):
    # this is 2-3 short paragraphs written for someone evaluating brands
    # in this category, not someone evaluating monitoraeo. Generated once
    # per refresh by industry_narrative.generate_buyer_intro(). Shape:
    #   {generated_at, model, paragraphs: [str]}.
    # Empty dict for pre-buyer-intro rows; template falls back to no intro.
    buyer_intro: dict[str, Any] = Field(default_factory=dict, sa_column=Column(JSONB))
    # noindex=True hides this ranking from public search engines and excludes
    # it from sitemap.xml. Auto-set by refresh_industry: True when NO brand
    # scored any visibility or citation (page is genuinely empty / AI didn't
    # name anything in the curated list), False when any brand scored.
    # Page still renders at 200 OK — users with the direct URL still see it.
    noindex: bool = Field(default=False)
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
...
class IndustryBrandHistory(SQLModel, table=True):
    ...
    gainers/losers vs the previous refresh. One row per brand per refresh.

    Kept separate from IndustryBrand (which only holds the current snapshot)
    so the trend history can grow indefinitely without bloating the live
    ranking query."""
    __tablename__ = "monitor_industry_brand_history"
    ...
    audited_at: datetime = Field(default_factory=datetime.utcnow, index=True)
